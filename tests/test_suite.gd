...
func _test_time_layering() -> void:
	...
	_check(
		_approx(
			morale_probe.morale,
			morale_probe.max_morale / float(Combat.MORALE_RECOVERY_DAYS)
		),
		"士气恢复必须进入每日时钟：第1天恢复上限的十分之一"
	)
	for _day in range(Combat.MORALE_RECOVERY_DAYS - 1):
		morale_sim._advance_day()
	_check(
...
func _test_three_way_siege() -> void:
	...
	_check(siege != null and siege.side_b_defends_city, "A 抵达应建带守军 SIEGE")
...
func _test_siege_arrival_triggers() -> void:
	...
	var siege := sim._siege_battle_of(city)
	_check(siege != null and not siege.side_b_defends_city and siege.side_b.is_empty(), "空城应建无防守军 SIEGE")
	_check(
	...
	_check(siege3 != null and siege3.side_b_defends_city and siege3.side_size(siege3.side_b) >= 1, "应建带守军 SIEGE")
	...
	_check(
		siege5 != null
			and siege5.side_b_defends_city
...
func _test_heightmap_heavy_transport_footprint() -> void:
	...
	_check(
		gs.armies.size() == 1
			...
			and heavy.max_morale == Army.DEFAULT_MAX_MORALE
...
func _test_siege_food_clock() -> void:
	...
	siege.side_b.append(garr); siege.side_b_defends_city = true
	...
	_check(
		Combat.SIEGE_STARVE_DEF_MULT < 1.0,
		"粮尽必须降低城市守军战斗效率"
	)
	sim.free()
...
func _test_morale_retreat_recovery() -> void:
	...
	_check(
		city68_battle != null
			and city68_battle.side_b_defends_city
...
func _test_retreat_transport_footprint() -> void:
	...
	remnant.max_morale = Army.DEFAULT_MAX_MORALE
	...
	_check(
		gs.armies.size() == 1
			...
			and remnant.max_morale == Army.DEFAULT_MAX_MORALE
...
func _test_capital_capture_capitulation() -> void:
	...
	var captured_capital_id := int(
		surrender_state.administrative_center_city_ids[0]
	)
	var captured_region := surrender_state.administrative_members(
		captured_capital_id
	)
	var retained_city_id := -1
	var victor_city_id := -1
	var third_city_id := -1
	for candidate in surrender_state.cities:
		if candidate.is_dock or captured_region.has(candidate.id):
			continue
		if retained_city_id < 0:
			retained_city_id = candidate.id
		elif victor_city_id < 0:
			victor_city_id = candidate.id
		elif third_city_id < 0:
			third_city_id = candidate.id
			break
	...
	surrender_state.cities[victor_city_id].owner_nation = 0
	surrender_state.recognized_city_owners[victor_city_id] = 0
	var loser_city_ids := captured_region.duplicate()
	loser_city_ids.append(retained_city_id)
	for loser_city in loser_city_ids:
		surrender_state.cities[loser_city].owner_nation = 1
		surrender_state.recognized_city_owners[loser_city] = 1
	_set_single_warehouse(surrender_state, 0, victor_city_id, 100)
	_set_single_warehouse(surrender_state, 1, captured_capital_id, 100)
	_set_single_warehouse(surrender_state, 2, third_city_id, 100)
	_set_warehouses(
		surrender_state,
		3,
		[] as Array[int],
	...
	capital_captor.location_city = victor_city_id
	capital_captor.move_from = victor_city_id
	surrender_state.armies.append(capital_captor)
	var expected_capital_transfers := (
		surrender_sim._capital_capture_transfer_city_ids(
			1, captured_capital_id, 0
		)
	)
	var capital_capture_revision_before := surrender_state.ownership_revision
	surrender_sim._capture_city(
		capital_captor,
		surrender_state.cities[captured_capital_id]
	)
	var captured_capital_transferred := (
		surrender_state.cities[captured_capital_id].owner_nation == 0
		and surrender_state.recognized_owner_of(captured_capital_id) == 0
		and surrender_state.cities[captured_capital_id].loyalty_target_nation == 0
		and surrender_state.cities[captured_capital_id].rebellion_progress == 0
		and surrender_state.cities[captured_capital_id].rebellion_cooldown_until_day
			> surrender_state.day
	)
	var captured_region_transferred := true
	for transferred_city in expected_capital_transfers:
		captured_region_transferred = (
			captured_region_transferred
	...
	var outside_city_retained := (
		surrender_state.cities[retained_city_id].owner_nation == 1
		and surrender_state.recognized_owner_of(retained_city_id) == 1
		and surrender_state.cities[retained_city_id].occupation_sponsor_nation == -1
	)
	...
	_check(
		captured_capital_transferred
			and captured_region_transferred
			and outside_city_retained,
		"首都州治失陷后该州须整体归胜方，州外城市保留给战败国"
	)
	_check(
		expected_capital_transfers == captured_region
			and surrender_state.ownership_revision
				== capital_capture_revision_before + 2,
		(
			"首都所在州须原子提交，随后和平结算独立提交"
			+ "（revision %d→%d）"
		) % [
			capital_capture_revision_before,
			surrender_state.ownership_revision,
		]
	)
	_check(
	...
	_check(
		surrender_state.nations[1].capital_city_id == retained_city_id
			and surrender_state.cities[
	...
	for stable_city in surrender_state.cities:
		if stable_city.id == captured_capital_id:
			continue
	...
	permanent_cession_held = (
		permanent_cession_held
		and surrender_state.cities[captured_capital_id].owner_nation == 0
		and surrender_state.recognized_owner_of(captured_capital_id) == 0
		and surrender_state.cities[captured_capital_id].loyalty_target_nation == 0
	)
	for transferred_city in expected_capital_transfers:
	...
	permanent_cession_held = (
		permanent_cession_held
		and surrender_state.cities[retained_city_id].owner_nation == 1
		and surrender_state.recognized_owner_of(retained_city_id) == 1
	)
	_check(
		permanent_cession_held
		...
		"首都州割让永久确认后推进三个忠诚月不得和平归还，州外城市仍属旧国"
	)
	surrender_sim.free()
	...
	_check(
		enclave_state.cities[16].owner_nation == 2
			and enclave_state.recognized_owner_of(16) == 2
...
func _test_ai_strategic_map_and_threat() -> void:
	...
	var startup_cached_nations: Array = startup_sim._ai_strategy_cache.keys()
...
func _test_ai_merge_and_retreat_utility() -> void:
	...
	full_source.max_size = GameState.INITIAL_HEAVY_ARMY_SIZE
...
func _test_manpower_pool_and_force_commands() -> void:
	...
	for city in gs.cities_of(nation_id):
		...
		if gs.city_administrative_output_enabled(city.id):
			monthly_income += Simulation.city_manpower_output(gs, city)
		_check(
	...
	gs.nations[nation_id].manpower_pool = 16000
	gs.nations[nation_id].treasury_gold = 200
	var formation_upkeep := GameState.army_monthly_upkeep(
		GameState.INITIAL_HEAVY_ARMY_SIZE
	)
	...
	_check(created != null and created.size == 15000 and created.max_size == 15000
		and gs.nations[nation_id].manpower_pool == 1000
		and formation_creation_cost == formation_upkeep * 10
		and gs.nations[nation_id].treasury_gold
			== treasury_before_creation - formation_creation_cost,
		"主战军建军应消耗15000人和10个月维护费：upkeep=%d create=%d treasury=%d"
			% [
				formation_upkeep,
				formation_creation_cost,
				gs.nations[nation_id].treasury_gold,
			])
	...
	finance_state.create_army(0, finance_city, 5000, 15000)
	...
	var cap_nation := 0
	var expected_cap := maxi(
		cap_state.land_cities_of(cap_nation).size()
			* GameState.ARMY_COUNT_LIMIT_PER_CITY,
		GameState.ARMY_COUNT_LIMIT_PER_CITY
	)
	_check(
...
func _test_war_preparation_route_block_grace() -> void:
	...
	var objective_city := int(objective.get("tactical_city_id", -1))
	...
	_check(
		not retarget_action.is_empty()
			...
			and int(retarget_action["objective_center_city"]) >= 0,
		"原目标道路封闭后必须保持对同国备战并改选可达州内战术城市"
	)
...
func _test_alliance_war_coalitions() -> void:
	...
	_check(
		rebellion_core >= 0
			...
			and rebellion_peace_state.recognized_owner_of(rebellion_core)
				== rebellion_peace_state.cities[rebellion_core].owner_nation
	...
	_check(
		joined
			...
			) == join_state.administrative_center_of(join_target)
...
func _test_suzerainty_invariants() -> void:
	...
	# 选国 0 的完整非首都州分封。
	var overlord_id := 0
	var enfeoff_cities := _enfeoffable_region(gs, overlord_id, 2)
	...
	# 分封不创造或转移野战军；藩王之后用自身资源组建主战军团。
	var vassal_army_count := 0
	for army in gs.armies:
	...
	_check(
		...
		"分封不得凭空创造或转移野战军（军队=%d 战团=%d）"
			% [vassal_army_count, gs.nations[subject_id].battle_groups.size()]
	)
	_check(
	...
	var transfer_region := _enfeoffable_region(
		transfer_state, transfer_overlord, 3
	)
	var owners_before: Dictionary = {}
	for army in transfer_state.armies:
		owners_before[army.id] = army.owner_nation
	...
	for army in transfer_state.armies:
		if int(owners_before.get(army.id, -1)) != army.owner_nation:
			owners_unchanged = false
			break
	_check(
		transfer_subject > 0
			and owners_unchanged
...
func _test_enfeoff_ai() -> void:
	...
	for city_id in finance_region:
		finance_state.cities[city_id].gold_per_month = 0
	...
	_check(finance_retained_city >= 0, "分封后宗主必须保留直属领地")
	...
	_enfeoffable_region(loss_state, 0, 3)
	...
	_check(
		...
		(
			...
			+ "（region=%d fiscal=%d burden=%.3f pressured=%d pressure=%.3f triggered=%s reason=%s）"
		) % [
			loss_region.size(),
			int(loss_report["monthly_fiscal_benefit"]),
			float(loss_report["burden_ratio"]),
			int(loss_governance["pressured_city_count"]),
			float(loss_governance["pressure_score"]),
			str(loss_triggered),
			loss_reason,
		]
	)
	...
	_enfeoffable_region(shallow_state, 0, 3)
	...
	_check(
		...
		(
			...
			+ "（region=%d fiscal=%d burden=%.3f pressured=%d pressure=%.3f triggered=%s）"
		) % [
			shallow_region.size(),
			int(shallow_report["monthly_fiscal_benefit"]),
			float(shallow_report["burden_ratio"]),
			int(shallow_governance["pressured_city_count"]),
			float(shallow_governance["pressure_score"]),
			str(shallow_triggered),
		]
	)
...
func _test_civil_war_relations() -> void:
	...
	civil_siege.side_b_defends_city = true
...
func _test_centralization_decision() -> void:
	...
	var stale_conqueror_action := (
		conqueror_action.duplicate(true)
		if not conqueror_action.is_empty()
		else {
			"kind": DiplomacyAI.Action.CENTRALIZE,
			"a": 0,
			"b": conqueror_subject,
...
func _test_civil_war_annexation() -> void:
	...
	if vassal_captor == null:
		var winner_capital := vs.nations[winner].capital_city_id
		vassal_captor = vs.create_army(
			winner,
			winner_capital,
			GameState.INITIAL_HEAVY_ARMY_SIZE,
			GameState.INITIAL_HEAVY_ARMY_SIZE
...
func _test_vassal_governance_output_bonus() -> void:
	...
	# 在同一座藩王城市上临时移除宗藩记录，隔离治理倍率，不破坏州治与属府同方控制。
	var probe: City = null
	for city in gs.land_cities_of(subject):
		if city.gold_per_month > 0 and city.food_per_half_year > 0:
			probe = city
			break
	_check(probe != null, "需要一座有正钱粮产出的藩王城作为探针")
	probe.war_disruption_until_day = 0
	for army in gs.armies:
		if not army.on_edge and army.location_city == probe.id:
			army.size = 0
	var subject_record: Dictionary = gs.suzerainty[subject].duplicate(true)
	gs.suzerainty.erase(subject)
	var independent_gold := Simulation.city_gold_output(gs, probe)
	var independent_food := Simulation.city_food_output(gs, probe)
	_check(
		is_equal_approx(Simulation.city_governance_output_multiplier(gs, probe), 1.0)
			and independent_gold > 0
			and independent_food > 0,
		"独立国城市不应用藩王治理加成"
	)

	gs.suzerainty[subject] = subject_record
	...
	_check(
		is_equal_approx(
			...
			and vassal_gold == int(floor(float(independent_gold) * 1.5))
			and vassal_food == int(floor(float(independent_food) * 1.5)),
		"藩王城钱粮产出须为 1.5×（gold %d→%d、food %d→%d）"
			% [independent_gold, vassal_gold, independent_food, vassal_food]
	)
	...
	if departed_target != null:
		...
		_check(
			bool(subject_removed.get("ok", false))
				...
				and departed_target.owner_nation
					== departed_state.recognized_owner_of(departed_target.id)
				and departed_target.occupation_sponsor_nation == -1
...
func _test_vassal_local_main_command() -> void:
	...
	for army in gs.armies:
		if army.owner_nation == subject and army.size > 0:
			army_n += 1
	_check(
		army_n == 0 and gs.nations[subject].battle_groups.is_empty(),
		"分封不赠送野战军（军队=%d、战团=%d）"
			% [army_n, gs.nations[subject].battle_groups.size()]
	)
	...
	if vassal_group == null or autonomous_main == null:
		sim.free()
		return
	var vassal_main := autonomous_main
	_check(
		vassal_main != null
			and vassal_main.owner_nation == subject
			and vassal_main.is_main_battle_role()
			and gs.battle_group_members(subject, vassal_group.id).size() == 1,
		"藩王应能以本国资源组建自有单重军 MAIN 战团"
	)
	if vassal_main == null:
		sim.free()
		return
	...
	for neighbor in gs.neighbors(cap):
		if gs.cities[neighbor].owner_nation == subject:
			relief_city = neighbor
			break
	_check(relief_city >= 0, "藩王战役测试需要第二座封地城市")
	var enemy_id := 1
	gs.set_diplomatic_relation(
		subject,
		enemy_id,
...
func _test_stranded_hostile_army_eviction() -> void:
	...
	gs.set_diplomatic_relation(0, 1, GameState.DiplomaticRelation.WAR)
	gs.refresh_derived()
	# 人为制造滞留：国 0 的一支主战军 IDLE 定居在敌城1（无通行权）。
	var stranded := _make_army(9301, 0, GameState.INITIAL_HEAVY_ARMY_SIZE, 10)
...
func _test_vassal_wartime_support_and_capital() -> void:
	...
	# 给前线藩王一支静止在本土的主战军团，验证战时仍归藩王自主指挥。
	# 复用网格生成时已有的军队并迁到封地首都，避免额外改变本测试的军队数量。
	var heavy: Army = null
	for army in gs.armies:
...
func _test_peacetime_demobilization_and_border_defense() -> void:
	...
	if concentrated_enemy != null:
		...
		concentrated_enemy.size = GameState.INITIAL_HEAVY_ARMY_SIZE
		concentrated_enemy.max_size = GameState.INITIAL_HEAVY_ARMY_SIZE
...
func _test_resource_hubs_and_food_mobilization() -> void:
	...
	if resource_target != null:
		...
		resource_target.manpower_per_month = 100000
		var selected := DiplomacyAI.select_war_objective(gs, 0, 1)
		_check(
			int(selected.get("city_id", -1))
				== gs.administrative_center_of(resource_target.id)
			and str(selected.get("reason", "")).contains("粮食核心")
			and str(selected.get("reason", "")).contains("人口核心"),
			"高产粮食/人口城市所属州应成为优先目标：target=%d center=%d selected=%s"
				% [resource_target.id, gs.administrative_center_of(resource_target.id), str(selected)]
		)
	...
	_check(
		not mobilized
			...
			and gs.nations[0].manpower_pool
				== GameState.INITIAL_HEAVY_ARMY_SIZE,
		"最后城市已有主战军时不得再生成旧式额外填线预备队"
	)
	_check(
		siege.has_army(guard)
			and guard.battle_id == siege.id,
		"已有主战守军必须继续留在围城守军侧"
	)
	# 资源充足也不得继续扩大小国的单重军机动预备队。
...
func _make_atomic_coalition_peace_fixture(seed: int) -> Dictionary:
	...
	for center_value in gs.administrative_center_city_ids:
		var candidate := gs.cities[int(center_value)]
		if (
			candidate.owner_nation == 1
			and not candidate.is_capital
			and not candidate.has_warehouse
		):
...
func _enfeoffable_region(
	...
	for center_id in centers:
		if center_id == capital_id or gs.cities[center_id].owner_nation != nation_id:
			continue
		var members := gs.normalize_enfeoff_region(
			nation_id,
			[center_id] as Array[int]
		)
		if members.is_empty():
			continue
		...
		if result.size() >= minimum_city_count:
			break
	if result.is_empty():
		for center_id in centers:
			if center_id == capital_id:
				continue
			var donor_region := gs.administrative_members(center_id)
			if donor_region.is_empty() or donor_region.has(capital_id):
				continue
			var donor_owner := gs.cities[center_id].owner_nation
			if (
				donor_owner == nation_id
				or (
					donor_owner >= 0
					and gs.suzerainty_root(donor_owner)
						== gs.suzerainty_root(nation_id)
				)
			):
				continue
			var operations: Array[Dictionary] = []
			for city_id in donor_region:
				operations.append({
					"city_id": city_id,
					"controller_id": nation_id,
					"legal_owner_id": nation_id,
					"sponsor_id": -1,
					"reset_political_target": true,
					"stock_policy": GameState.TerritoryStockDisposition.RETURN_TO_OLD_POOL,
					"reason": "test_enfeoffable_region",
				})
			var transfer := gs.apply_territory_transaction(operations)
			if bool(transfer.get("ok", false)):
				result = donor_region
				for _closure_pass in range(gs.administrative_region_count + 1):
					var closure := gs.enfeoff_region_closure(nation_id, result)
					var complete: Dictionary = {}
					for city_id in closure:
						var closure_center := gs.administrative_center_of(city_id)
						for member_id in gs.administrative_members(closure_center):
							complete[member_id] = true
					var complete_ids: Array[int] = []
					complete_ids.assign(complete.keys())
					complete_ids.sort()
					var completion_operations: Array[Dictionary] = []
					for member_id in complete_ids:
						if (
							gs.cities[member_id].owner_nation == nation_id
							and gs.recognized_owner_of(member_id) == nation_id
						):
							continue
						completion_operations.append({
							...
							"reset_political_target": true,
							"stock_policy": GameState.TerritoryStockDisposition.RETURN_TO_OLD_POOL,
							"reason": "test_enfeoffable_closure",
						})
					if not completion_operations.is_empty():
						var completion := gs.apply_territory_transaction(
							completion_operations
						)
						if not bool(completion.get("ok", false)):
							result.clear()
							break
					result = gs.normalize_enfeoff_region(nation_id, complete_ids)
					if not result.is_empty():
						break
				if not result.is_empty():
					break
	result.sort()
	if result.size() >= gs.land_cities_of(nation_id).size():
		return [] as Array[int]
	return result
...
## 构造一场攻城：攻方在城墙(dist=L)、守军在城中(dist=0)。
func _make_siege_battle(attackers: Array, defender: Army, _legacy_fort: int, distance: int) -> Battle:
	var b := Battle.new()
	b.id = 0
	b.kind = Battle.Kind.SIEGE
	b.edge = _make_edge(0.0, distance)
	b.city = City.new()
	b.side_b_defends_city = true
