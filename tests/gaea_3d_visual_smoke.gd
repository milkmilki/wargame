extends SceneTree
## Deterministic screenshot probe for the Gaea strategic map.
...
func _run() -> void:
	...
	while (
		...
		if Time.get_ticks_msec() - started > TIMEOUT_MSEC:
			push_error("GAEA_3D_VISUAL_TIMEOUT")
			quit(1)
			return
		await process_frame
	...
	if output.is_empty():
		output = "/tmp/world-war-gaea-3d.png"
	var image := root.get_texture().get_image()
	if image == null or image.is_empty():
		push_error("GAEA_3D_VISUAL_EMPTY")
		quit(1)
		return
	var error := image.save_png(output)
	if error != OK:
		push_error("GAEA_3D_VISUAL_SAVE_FAILED:%d" % error)
		quit(1)
		return
	print(
		"GAEA_3D_VISUAL_OK path=",
		output,
		" size=",
		image.get_width(),
		"x",
