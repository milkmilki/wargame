extends SceneTree
## Geometry gate for dock-to-land-city spacing. Docks are generated from road
## crossings and must remain visually separate from every ordinary city.
...
func _init() -> void:
	...
	for edge in state.edges:
		if edge.max_manpower <= 0 or edge.kind in [Edge.Kind.RIVER, Edge.Kind.SEA]:
			continue
		var road_start := state.cities[edge.city_a].map_position
		var road_end := state.cities[edge.city_b].map_position
		...
		for river_id in range(state.river_paths.size()):
			...
			for segment_index in range(river.size() - 1):
				var hit = Geometry2D.segment_intersects_segment(
					road_start, road_end, river[segment_index], river[segment_index + 1]
				)
				if hit == null:
					continue
				var t := (Vector2(hit) - road_start).dot(road_delta) / road_delta.length_squared()
				if t > TerrainMapGenerator.RIVER_CROSSING_ENDPOINT_EPS and t < 1.0 - TerrainMapGenerator.RIVER_CROSSING_ENDPOINT_EPS:
					crossing_count += 1
					print("DOCK_CITY_CROSS edge=", edge.city_a, "-", edge.city_b, " kind=", edge.kind, " river=", river_id, " t=", t, " pos=", road_start, " -> ", road_end, " hit=", hit)
