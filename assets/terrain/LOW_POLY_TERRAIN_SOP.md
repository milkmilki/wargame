...
- 与高程图完全同范围、同投影、同像素对齐的卫星图或手绘底色图。
- 没有底色图时，工具只生成中性陆色和海深渐变；不会生成高程伪彩色。
...
1. 从 Alpha 解码高程。
2. 采样为 `256 × 按地图宽高比缩放` 的共享顶点低模网格；相对 2048 源
   仍是约 1/8 线性分辨率，但能保留海岸和小岛。高程采样 UV 与网格顶点 UV 完全一致。
...
政治覆色 100% 时不泄露卫星 RGB；关闭“地形塑形光”后坡面方向性明暗应明显减弱。
海洋作为无主政治区域，与国家颜色使用同一覆色百分比：混合模式不会突然切成
全强度深蓝，100% 政治模式才显示完整海洋色。
政治模式下所有填色都直接落在同一个低模表面；卫星、政治色、无主海洋色不是
额外的平面。省界、国界、海岸、道路和河流则按同一低模高度采样后贴地绘制。
