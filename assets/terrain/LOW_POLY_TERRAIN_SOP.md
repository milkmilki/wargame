...
Godot 在运行时确定性生成分面低模网格，底色来自同坐标表面图或中性陆海色，
体积感完全由网格法线与双灯光产生。
...
1. 从 Alpha 解码高程。
2. 采样为 `160 × 按地图宽高比缩放` 的网格。
3. 将高度量化为 128 级，抑制噪点但不形成明显梯田。
4. 每个三角形复制三个顶点并写入独立面法线，不做平滑法线。
...
验收重点：海岸线仍贴合 0 米；山区可见三角切面；平原没有高程纹理噪声；
政治覆色 100% 时不泄露卫星 RGB；关闭“地形塑形光”后切面方向性明暗应明显减弱。
