...
- `china_elevation_white_2048.png`: the single packed runtime map source. RGB is
  solid white; Alpha is co-registered numeric elevation
...
At runtime, Godot reads geography from the Alpha channel and renders land with
a white material. No procedural terrain plugin or network map service is
required at runtime.
...
For a user-supplied heightmap and the deterministic smooth-shaded low-poly lighting
...
`china_elevation_white_2048.json`.
