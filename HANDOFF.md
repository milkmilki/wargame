...
> 最后更新：2026-08-06（全面攻势、种田区位经济、地图缩放平移、码头间距与重军双倍士气闭环；战斗系统重构见 [COMBAT_REFACTOR_CHANGES.md](COMBAT_REFACTOR_CHANGES.md)）。
...
| [scripts/model/nation.gd](scripts/model/nation.gd) | 数据 | 国家资源、首都粮仓、外交解释字段、战争动员目标、持久战区锚点、战役梯队、多目标准备分配及 180 天满攻势准备状态 |
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 1167 断言，含全军维护费/建制费/攻势费、160 城经济容量、平原/港市/枢纽与一跳传播、地图锚点缩放和平移边界、码头最小间距、重军双倍士气、无交叉平面道路、外交联盟残局、河运、动态多槽防区、四路以上全面攻势、战团全员集结、多路攻城、小国动员、孤城围歼及既有门禁，headless 运行 |
...
- **主战/填线任务所有权**：战团成员统一为 `MAIN`，承担完整 Utility AI、国家战役、主动攻击和重点城市机动解围；战团外 `5000` 军为 `LINE`，只执行持久防区 Assignment，不自行攻击。每个攻势城市目标必须且只能绑定一个战团，不再临时借用其他填线军；同一波可由不同战团并行攻击不同目标。满编战团的三支军优先占用三个不同集结方向，地图拓扑不足时不强造三路。自动合并按战团与防区隔离，重军拆为轻军时同团最多保留两支。
...
| 正式 AI 只有固定进攻性，无法配置国家风险偏好；纯僵持时宣战与外交动作不足 | `Nation.ai_aggression` 成为国家级唯一配置源，正式模式按 `0.5～1.5` 截断后同时影响宣战收益、战术进攻/撤退阈值和攻势重整周期；默认 `1.0` 保持镜像一致。全局宣战线为 `0.85`，国家级攻势线为 `1.00`，国家级攻势周期为 30 天；普通逐军攻击线仍为 `1.35` |
...
- 快速回归 `./run_tests.sh` = **1165 passed / 0 failed**（含无交叉平面道路、码头最小间距、真实地图目标经济容量、重军双倍士气、动态防区、四路以上全面攻势、战团全员集结、道路终点溃退军先落位、多路攻城、填线/主战隔离、小国动员、孤城围歼及既有财政、160 城、外交、河运、战斗和 UI 门禁）。
