...
两阶段统一提交后，使用新 1.5 倍城市数军制的完全镜像固定种子十年结果为严格 `32:32`，双方有效战力 `474008.2/474008.2`、粮食 `170174/170174`、优势分 `0.0`。旧版依赖国家遍历顺序产生战局分叉的左右 A/B 数字已失效；攻击合法路径由专项不可达纵深目标夹具验证。后续比较策略时必须显式加入可交换的小扰动或使用多种子非完全对称场景，不能把调度偏差当成 AI 能力。

最新回归为 `673 passed, 0 failed`。真实地图长跑为 4 种子 × 1095 天，四局均保持 4 国存活，战争声明从上一基线 2 次提高到 5 次，领土变化为 `1/0/1/1`，联盟均为 2 次，最终断粮军均为 0，`invalid=0` 且 `commit_failures=0`，总耗时约 `37.7s`。纯和平 1080 天四国精确维持目标军制且年度粮食结余均为正。十年 A/B 中改进 AI 优势分为 `+5833.2`；严格镜像继续保持 `32:32`、优势分 `0.0`。
