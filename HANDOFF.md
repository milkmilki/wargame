...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **829 断言 / 0 失败**）。
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 829 断言，含外交三层态度/联盟残局、双河模板/渡口压缩、真实边距、战略图交互、邻接驰援、双边议和/多国灭国投降、两档军制与同城多军驻防门禁，headless 运行 |
...
- 快速回归 `./run_tests.sh` = **829 passed / 0 failed**（含外交三层态度分解、两国联盟残局退盟/拒绝复盟/转入统一战争、战略图四档缩放、黄河 `0.50～0.60` 门禁、多国灭国投降、真实边距、邻接驰援和同城多军驻防）。
...
- 含新外交态度与统一目标的正式地图 4 种子 × 1095 天：各种子 `eliminated_wars=0`、`terminal_alliance_lock=0`，且 `invalid=0`、`commit_failures=0`；`net_captures=88`、`turnovers=136`、wars 13、offensives 76。共出现 9 个多目标准备计划，最大实际并行方向数为 2；城市数稳定超过 30 天的军制偏差为 0。非插桩总耗时约 `86.0s`。
