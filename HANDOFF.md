...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **1112 断言 / 0 失败**）。
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 1112 断言，含 160 城低地/中东南/河岸密度、外交三层态度/联盟残局、双河模板/渡口压缩、真实边距、战略图交互、邻接驰援、双边议和/多国灭国投降、两档军制与同城多军驻防门禁，headless 运行 |
...
- **性能**：`AiWorldView` 建立军队与驻军战力索引；AI 决策周期复用路径场；`ThreatField` 复用拓扑传播距离；补给从粮仓反向构建国家网络；Dijkstra 使用确定性二叉最小堆 `O((V+E)logV)`。64 陆城旧基准约 `86～95s`；正式地图扩展到 160 陆城后，四种子 × 1095 天约 `405.2s`，末期军队为 100～101。规模增加有明确运行成本，不得继续引用旧基准。
...
- 快速回归 `./run_tests.sh` = **1112 passed / 0 failed**（含 160 城规模、低海拔/中东南密度梯度、至少 10% 河岸陆城、动态间距、四国各 40 城内部连通，以及既有外交、河运、战斗和 UI 门禁）。
...
- 160 陆城正式地图 4 种子 × 1095 天：各种子 `eliminated_wars=0`、`terminal_alliance_lock=0`，且 `invalid=0`、`commit_failures=0`；`net_captures=17`、`turnovers=93`、wars 13、offensives 40。共出现 2 个多目标准备计划，最大实际并行方向数为 3；城市数稳定超过 30 天的军制偏差为 0。非插桩总耗时约 `405.2s`。
