...
> 最后更新：2026-08-08（**战争态性能收敛**：补给/补员/填线分帧、AI 决策错峰、渲染墙钟插值、投降飞地割让修复；见 §9。历史战斗系统重构见 [COMBAT_REFACTOR_CHANGES.md](COMBAT_REFACTOR_CHANGES.md)）。
>
> **接手第一件事**：读 §9（性能现状与剩余优化路线图）和 §10（40 国统一收敛）。当前 `./run_tests.sh` = **1217 passed / 0 failed**，strict-mirror 连续 3650 天优势分 `0.0`。
...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **1217 断言 / 0 失败**）。
...
- 当前快速回归为 **1195 passed / 0 failed**；strict-mirror：seed 1 连续 **3650 天逐日无破裂**，优势分 `0.0`。
