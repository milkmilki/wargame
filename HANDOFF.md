...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 1170 断言，含两步路线冻结、无合法第二步释放准备、重军零士气终止、正式地图普通攻击门禁、断粮突围、全军维护费/建制费/攻势费、160 城经济容量、种田传播、地图导航、重军双倍士气、平面道路、动态防区、全面攻势及既有门禁，headless 运行 |
...
| `MORALE_RECOVER` | 0.15 | 非交战有粮军队每月士气恢复量（战后疲劳消退，封顶各军 `max_morale`） |
...
5. **恢复资源**：每 30 天恢复至多 `MORALE_RECOVER=0.15`；从损耗最低的可达粮仓取粮，完整恢复月基础需求为 `ceil(size × RECOVERY_FOOD_PER_CAPITA)`，再计运输损耗。`RECOVERING` 不进入普通 `_resolve_supply`，避免双扣。
...
- 普通城市 `manpower_per_month=10～30`。正式地图陆城基础范围为 `gold_per_month=2～5`、`food_per_half_year=145～195`，网格夹具保留旧经济范围；真实地图每国额外生成一个人口核心（至少 80/月）和一个粮食核心（至少 1600/半年）。种田重分配前基线为 `547 金/月、31974 粮/半年`，对应 `160×5000` 轻军与 `40×15000` 重军的 `520 金/月、31500 粮/半年` 目标支出。
- **种田区位经济**：真实地图生成道路后一次性派生静态地理权重。与码头直接相连的 28 座港市金币权重 `3.0×`；至少 6 条正容量陆路/抢滩路的 24 座高连接枢纽金币权重 `1.5×`；局部起伏最低 35% 的 56 座平原城金币与粮食权重 `1.5×`。直接加成的奖励部分只向相邻陆城传播一次、传播率 50%，多个来源取最大值，当前另有 65 座城市仅获得传播加成。最后按原始产出×权重做确定性整数配额，粮食核心保留最低 1600，正式地图全图总产出保持 `547 金/月、约 32015 粮/半年`，不会因区位系统凭空增产。
...
- 快速回归 `./run_tests.sh` = **1170 passed / 0 failed**（含两步路线冻结、无合法第二步释放准备、重军零士气终止、正式地图普通攻击关闭而断粮突围保留，以及无交叉道路、目标经济容量、重军双倍士气、动态防区、全面攻势和既有门禁）。
