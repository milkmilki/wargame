...
两阶段统一提交后，完全镜像固定种子十年结果为严格 `32:32`，双方有效战力 `49209.1/49209.1`、粮食 `978/978`、优势分 `0.0`。旧版依赖国家遍历顺序产生战局分叉的左右 A/B 数字已失效；攻击合法路径由专项不可达纵深目标夹具验证。后续比较策略时必须显式加入可交换的小扰动或使用多种子非完全对称场景，不能把调度偏差当成 AI 能力。

最新回归为 `490 passed, 0 failed`。真实地图长跑为 4 种子 × 1095 天，四局均保持 4 国存活，领土变化次数为 `3/1/3/1`，最终粮仓总库存为 `7474/6743/7859/9037`，断粮军为 `0/0/1/0`，全部军队有有效命令，`invalid=0` 且 `commit_failures=0`。纯和平 1080 天四国年度粮食结余均为正。四种子总耗时约 `43s`；命令缓冲为 `O(A)` 内存、提交排序为 `O(A log A)`，威胁场和城市防御计划均每国只构建一次。军制调整在冻结快照后结算，其变化从下一次 AI 决策（最多 5 天）起进入军事规划。
