...
从带 Alpha 的灰度高度图生成 64 个陆城、道路、河流和动态码头，4 国各 16 个连续陆城，真实地图和平开局；AI 自动完成外交、河陆寻路、交战与占领，直到一国统一。
...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **826 断言 / 0 失败**）。
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 826 断言，含双河模板/渡口压缩、真实边距、邻接驰援、双边议和/全境投降、两档军制与同城多军驻防门禁，headless 运行 |
| [tests/map_visual_smoke.gd](tests/map_visual_smoke.gd) | 视觉烟测 | 构造占领省份与攻势事件，用 Godot Movie Maker 验证真实渲染路径 |
| [tests/ai_longrun.gd](tests/ai_longrun.gd) | 诊断 | 4 种子 × 1095 天 AI 长跑，统计战争活动并校验城市数稳定 30 天后的两档军制精确收敛；支持旧版固定城防 A/B |
...
- **建军/解散**：AI 国家级命令 `CREATE_ARMY/DISBAND_ARMY`。正式地图分别比较轻军/重军现有数量与城市比例目标，缺额时在未被围粮仓创建对应满编军，超额时从安全闲置军中整建制裁撤；战争动员不额外突破目标。
- **统一驻防目标**：首都、粮仓、资源核心和粮道只提供城市价值与补给重要性，不再生成固定人数门槛。威胁、解围需求和城市重要性统一进入 `CityDefensePlan.required_power`。
- **归一化防区与离散分配**：一线为实际/潜在前线，二线为可及时回援的一线邻城。城市重要度=`城市价值×(1+粮道重要度)`，危险度取本地威胁与按行军时间衰减的一线威胁最大值；国家防御预算为 `P×T/(P+T)`。每城槽位上限为 `ceil(该城需求/平均军队战力)`，新增槽位按未覆盖战力递减；Hungarian 匹配保证每军最多一个目标，但高需求城市可同时分配多军。
- **稳定任务所有权**：攻势集结只能抽调 `CityDefensePlan.can_redeploy()` 允许的军队，不能覆盖有效防御部署锁；不存在关键城市固定人数预留。跨城合并按 `EquivariantOrder` 势力局部物理序确定唯一接收方向，使目标图严格无环且不依赖军队 ID。
- **性能**：`AiWorldView` 建立军队与驻军战力索引；AI 决策周期复用路径场；`ThreatField` 复用拓扑传播距离；补给从粮仓反向构建国家网络；Dijkstra 使用确定性二叉最小堆 `O((V+E)logV)`。首次优化基准由 `313973ms` 降至 `152096ms`（`2.06×`）；渡口压缩基线约 `95.7s`，真实边距版本四种子非插桩长跑约 `100.4s`，末期军队为 45～46。
- **调度**：每日先合并；每 5 天重算威胁并决策；城市易主或整数城防恢复通过独立版本号触发下一轮战略图重算。所有国家性格由 nation id 确定生成，不训练、不引入随机不可复现性。
...
- 求和必须双边同意：至少一方达到 `1.25` 提议阈值，并且双方各自都达到 `0.60` 接受底线；合计分不能替代任何一方同意，战争满 900 天也不再强制停战。和平意愿由战争疲劳、战局、归一化军力、钱粮续航、第三国边境实际集结、多线战争和进攻性组成；占领敌方重要城市或拥有军力优势会降低和平意愿，失地、军力劣势、钱粮将尽和中立邻国压境会提高和平意愿。停战期仍为 180 天。
- 全境失守是战争结算硬规则：若双方都仅与彼此交战，任一方失去全部实际控制城市后立即投降，不等待月度外交 tick，也不经过胜方和平接受线；多国战争不触发该规则。结算复用普通和平路径，确认领土、结束战斗、清理战争目标并记录 `surrendering_nation`。
...
- 正式世界从 `china-map-...webp` 的 Alpha 最大连通区域提取陆地，在局部低起伏区域用确定性最远点采样生成 64 个陆城；按地图真实宽高比强制最小陆城间距 `0.075`。
...
- 高度图使用两组固定地理控制点生成北黄河、南长江：两河西向东，平均纵向间隔至少 `0.18`，局部向西折返不超过 `0.08`。路径代价惩罚偏离各自纬向走廊，避免重叠成一条线或在东北折返。
- 渡口先沿每条河均匀保留约 35% crossing，再用并查集补足维持全图连通所需的最少跨河道路，最终约为旧 35 座的一半（门禁 8～22）。保留道路的全部交点实体化为码头；其他穿河陆路禁用，因此减少渡口不会破坏河流硬阻隔。
- 河流是硬陆路障碍：任何可通行 `LAND/LANDING` 边都不得从河流内部穿过。跨河只能先进入码头，再通过抢滩边或相邻码头间水路移动；同一原道路若依次穿过两条河，必须依次经过两个码头。
- 码头是 `City`，具备占领、驻军、工事、补给和法理归属；初始产出为 0，不改变四国开局经济。首都和资源核心只从 64 个陆城选择；两档军制按全部当前城市（含码头）计算。
...
- 四国按空间均衡分区，每国严格 16 个陆城且本国正容量道路连通。首都选择本国陆城几何中心附近的城市。
...
- **高度图与河流**：渲染器裁切同一灰度图，并绘制弯曲蓝色河道、蓝色水路、红色虚线抢滩边和菱形码头；普通道路、陆城和军队保持前景层。
...
- 快速回归 `./run_tests.sh` = **826 passed / 0 failed**（含双河分离走向、渡口约半与最小连通、三类边真实几何距离、长距离行军不封顶、双边全境投降、普通城市邻接驰援、同城多军驻防和既有战斗/攻势门禁）。
...
- 含真实边距、双河与邻接驰援的真实地图 4 种子 × 1095 天：`invalid=0`、`commit_failures=0`；`net_captures=88`、`turnovers=137`、wars 17、offensives 63。共出现 3 个多目标准备计划，最大实际并行方向数为 3；城市数稳定超过 30 天的军制偏差为 0。非插桩总耗时约 `100.4s`。
- 当前并行容量按主方向最低到位兵力后的真实富余量推导，不固定占满三个方向。关键城市没有固定人数保底，实际受威胁驻防只由 `CityDefensePlan` 的比例预算和离散匹配决定。
