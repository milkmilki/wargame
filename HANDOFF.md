...
当前 A/B 开关只关闭“攻击候选合法路径预检”，其他 AI 完全相同。固定种子 `current-control` 的左侧优势分为 `+35389.4`；启用方在左时统一 64 城，优势分 `+149519.3`，相对对照改善 `+114129.9`；启用方在右时右侧以 `37:27` 反超，右侧优势 `+21416.1`，相对对照改善 `+56805.5`。位置校正后的配对净收益约 `+170935.4`，改进策略放在任一侧都占优。

最新回归为 `464 passed, 0 failed`。真实地图长跑为 4 种子 × 1095 天，四局均保持 4 国存活，领土变化次数为 `11/15/7/12`，备战次数为 `7/5/9/10`，自然宣战次数为 `5/3/4/4`；每局形成 `1～2` 条长期防御联盟，未冻结战争。最终粮仓总库存为 `6642/7821/7060/7094`，断粮军为 `0/0/0/1`，全部军队有有效命令且 `invalid=0`。纯和平 1080 天四国年度粮食结余均为正。
