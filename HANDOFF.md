...
> 最后更新：2026-08-06（动态边境防区、分层填线与最近路径补位闭环；战斗系统重构见 [COMBAT_REFACTOR_CHANGES.md](COMBAT_REFACTOR_CHANGES.md)）。
...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **1166 断言 / 0 失败**）。
...
| [scripts/view/map_renderer.gd](scripts/view/map_renderer.gd) | 渲染 | 只读 `_draw` 的二战战略图风格渲染；牛皮纸、省份/国境、持续攻势箭头、军图城市/码头、NATO 兵牌、四档 UI、兵牌大小滑块和城市/道路点击详情 |
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 1166 断言，含全军维护费/建制费/攻势费/军费士气恢复、160 城分布、无交叉平面道路、外交联盟残局、河运、真实边距、战略图交互、城名开关、轻重军兵牌差异、动态多槽防区分层填充、最近路径补位、受围整区召回、胜后归槽与失守协同撤退、持久战区滞回、长期和平压力、战团全员集结/半团拒绝绑定/360天退出、道路终点溃退落位与 68 城接战、多路攻城倍率、持久战团编组与补员同步、填线/主战职能隔离、小国生存动员、负收益征兵、孤城围歼与包围目标选择及既有门禁，headless 运行 |
...
- **持久战团**：`BattleGroup` 是主战编组真源，每团容量固定为 `2×5000 + 1×15000`，按“第一轻军 → 第二轻军 → 重军”顺序扩充；一支或两支轻军组成的不满编战团也可发动攻势，重军只能作为战团成员由国家建军入口创建。成员关系跨攻势保留，战损后的补员自动继承进行中的战团目标。和平期填线已满后只维护一个完整战团；备战或战争期按并行攻势需求扩至最多 3 个，禁止把资源无用途地持续堆成首都闲置战团。
...
- 战争中每 30 天进入下一轮国家级战役规划窗口。统一国家时钟下最多并行准备 3 个同战区敌方目标，`campaign_preparation_assignments` 冻结一军一目标归属，不能在多个方向重复计算兵力；达到门槛的方向可同批发动。主方向优先满足，再用剩余可调兵力建立邻近第二、第三方向，禁止把同一重军突击组同时拉向远端战区。
...
- 快速回归 `./run_tests.sh` = **1166 passed / 0 failed**（含无交叉平面道路、动态防区随控制区变化、全局多层填槽、最近路径补位、受围整区召回、胜后归槽、失守协同撤退、长期和平压力、战团全员集结宣战、半团拒绝绑定与360天退出、道路终点溃退军先落位、真实地图 68/72 城胜方进城接战、兵牌大小与城名开关、轻重军兵牌、持久战区/战团、多路攻城、填线/主战隔离、小国动员、负收益征兵、孤城围歼及既有财政、160 城、外交、河运、战斗和 UI 门禁）。
...
- 持久战团与动态填线军制加入后默认种子 1095 天：`armies=206`、`offensives=24`、`max_parallel=3`、`force_mismatches=0/0`、`role_deploy=197`、`max_idle_stack=8/8/7`、`hostile_stationed=0`、`commit_failures=0`，脚本 `total_ms=43363`。曾出现无用途扩军到 415 支、首都堆叠 67 支的失败版本，已通过“和平 1 团、备战/战争最多 3 团”门禁消除。
...
- 当前一个目标只能绑定一个战团，并行目标数由可用战团数自然限制为最多 3。关键城市没有固定人数保底，正式地图受威胁驻防由 `CityDefensePlan` 的角色槽位决定。
