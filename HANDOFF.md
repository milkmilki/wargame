...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **445 断言 / 0 失败**）。
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 445 断言，headless 运行 |
...
当前固定种子结果：A 主力优先 AI 在左/B 当前按 ID 决策 AI 在右为 `63:1`、左侧优势分 `+129762.9`；交换后左侧当前 AI 对右侧主力优先 AI 为 `3:61`、右侧优势分 `+115063.8`。配对净优势为 `+244826.7`，改进策略放在任一侧都占优。`current-control` 显示当前 AI 对当前 AI 会因固定迭代侧偏形成 `63:1`，所以禁止只用单局左胜下结论；必须同时运行 `improved-left/improved-right`。

最新真实地图长跑为 4 种子 × 1095 天，四局均保持 4 国存活，领土变化次数为 `10/7/8/10`。备战次数为 `6/7/10/13`，完成集结后的自然宣战次数为 `5/3/4/5`；四局共触发 41 支额外动员军。最终粮仓总库存为 `7448/8324/9175/6960`，断粮军为 `0/1/0/0`，全部军队有有效命令且 `invalid=0`。
