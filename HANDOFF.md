...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 826 断言，含双河模板/渡口压缩、真实边距、战略图四档缩放/统计窗口/点击命中、邻接驰援、双边议和/多国灭国投降、两档军制与同城多军驻防门禁，headless 运行 |
...
- **性能**：`AiWorldView` 建立军队与驻军战力索引；AI 决策周期复用路径场；`ThreatField` 复用拓扑传播距离；补给从粮仓反向构建国家网络；Dijkstra 使用确定性二叉最小堆 `O((V+E)logV)`。首次优化基准由 `313973ms` 降至 `152096ms`（`2.06×`）；黄河 `0.50～0.60` 版本四种子非插桩长跑约 `95.2s`，末期军队为 43～45。
...
- 渡口先沿每条河均匀保留约 35% crossing，再按正式地图空间分国结果用并查集补足每个初始国家内部连通所需的最少渡口，最后补足全图连通；码头数量仍受 8～22 门禁约束。保留道路的全部交点实体化为码头，其他穿河陆路禁用。
...
- 快速回归 `./run_tests.sh` = **826 passed / 0 failed**（含战略图四档缩放、国家统计窗口空间回收、城市/道路命中与持续箭头、黄河 `0.50～0.60` 门禁、双河走向、国家内部/全图最小连通、多国灭国投降、真实边距、邻接驰援和同城多军驻防）。
...
- 含 `0.50～0.60` 黄河、真实边距与邻接驰援的正式地图 4 种子 × 1095 天：各种子 `eliminated_wars=0`，且 `invalid=0`、`commit_failures=0`；`net_captures=116`、`turnovers=158`、wars 12、offensives 71。共出现 11 个多目标准备计划，最大实际并行方向数为 3；城市数稳定超过 30 天的军制偏差为 0。非插桩总耗时约 `95.2s`。
