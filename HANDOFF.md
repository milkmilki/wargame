...
当前 A/B 开关只关闭新增粮道守备，其他 AI 完全相同。固定种子 `current-control` 的左侧优势分为 `+69694.7`；启用方在左时优势分 `+35389.4`，相对对照 `-34305.3`；启用方在右时左侧优势分降为 `+33710.1`，即右侧相对对照改善 `+35984.6`。位置校正后的配对净收益约 `+1679.3`：没有强胜证据，但已排除宏观负收益。专项唯一粮道夹具负责证明确定性行为，不得用单局左胜代替双向结果。

最新回归为 `461 passed, 0 failed`。真实地图长跑为 4 种子 × 1095 天，四局均保持 4 国存活，领土变化次数为 `11/15/7/12`，备战次数为 `7/5/9/10`，自然宣战次数为 `5/3/4/4`；每局形成 `1～2` 条长期防御联盟，未冻结战争。最终粮仓总库存为 `6642/7821/7060/7094`，断粮军为 `0/0/0/1`，全部军队有有效命令且 `invalid=0`。纯和平 1080 天四国年度粮食结余均为正。
