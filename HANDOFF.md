...
- **轻重军任务所有权**：`15000` 编制军不进入常态驻防，作为国家战役机动军；正式地图主攻方向投入全部可用重军突击组，仅在存在唯一最优候选时追加一支邻近 `5000` 边槽军，城市槽填线军不可抽调。无重军时允许最多三支可抽调轻军组成临时攻势，避免少于 20 城的小国永久失去国家级进攻能力；若国家仍有重军编制目标但重军全灭，则优先重建第一支重军，之后恢复轻军优先顺序。宣战前集结的轻军数不得超过已投入重军数；邻城即时驰援最多两支 `5000`，不抽调重军。
...
- 快速回归 `./run_tests.sh` = **1129 passed / 0 failed**（含轻重军角色分离、零重军降级攻势、首支重军重建、敌占城市强制撤离、后到援军重新接战，以及既有财政、160 城、外交、河运、战斗和 UI 门禁）。
...
- 160 陆城正式地图 4 种子 × 1095 天：各种子 `eliminated_wars=0`、`terminal_alliance_lock=0`，且 `invalid=0`、`hostile_stationed=0`、`commit_failures=0`；`net_captures=17`、`turnovers=119`、wars 8、offensives 59。城市数稳定超过 30 天的军制偏差为 0；本轮总耗时 `676.4s`，高于上一轮，需保留为性能回归观察项。敌占城市的恢复/待机/容量等待撤退军会立即沿第一段道路强制撤离，旧城主后到援军会作为攻方重新触发攻城战。
