...
- 快速回归 `./run_tests.sh` = **1116 passed / 0 failed**（含全军月维护、10:1 建制费、攻势资金门槛、参与军队费用单调性、军费支付率士气公式，以及既有 160 城、外交、河运、战斗和 UI 门禁）。
...
- 160 陆城正式地图 4 种子 × 1095 天：各种子 `eliminated_wars=0`、`terminal_alliance_lock=0`，且 `invalid=0`、`commit_failures=0`；`net_captures=17`、`turnovers=93`、wars 13、offensives 40。共出现 2 个多目标准备计划，最大实际并行方向数为 3；城市数稳定超过 30 天的军制偏差为 0。非插桩总耗时约 `406.6s`。
