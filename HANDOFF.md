...
两阶段统一提交后，使用新 1.5 倍城市数军制的完全镜像固定种子十年结果为严格 `32:32`，双方有效战力 `475009.1/475009.1`、粮食 `287687/287687`、优势分 `0.0`。旧版依赖国家遍历顺序产生战局分叉的左右 A/B 数字已失效；攻击合法路径由专项不可达纵深目标夹具验证。后续比较策略时必须显式加入可交换的小扰动或使用多种子非完全对称场景，不能把调度偏差当成 AI 能力。

最新回归为 `660 passed, 0 failed`。真实地图长跑为 4 种子 × 1095 天，四局均保持 4 国存活，领土变化次数为 `1/4/0/1`，最终粮仓总库存为 `127623/126450/138719/136277`，最终断粮军均为 0，全部军队有有效命令，`invalid=0` 且 `commit_failures=0`。纯和平 1080 天四国精确维持目标军制且年度粮食结余均为正。四种子总耗时约 `202s`；严格镜像十年继续保持 `32:32`、优势分 `0.0`，耗时约 `371s`。军队实体由 64 增至 96 后逐军 AI 成本显著增加，继续扩大战争规模前应优先缓存跨军共享的战略规划和路径结果。
