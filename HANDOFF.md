...
`run_tests.sh` 两阶段：①headless 导入捕获脚本错误 ②运行 `tests/test_suite.gd`（当前 **783 断言 / 0 失败**）。
...
| 文件 | 角色 | 关键内容 |
|---|---|---|
| [scripts/model/city.gd](scripts/model/city.gd) | 数据 | 城市控制、地形、产出、粮仓、重点产地；当前/完整工事强度 `fort_strength/fort_strength_max` 及最近易手日 `fort_last_capture_day`；和平确认前冻结直接交战方的 `occupation_sponsor_nation` |
...
| [scripts/model/nation.gd](scripts/model/nation.gd) | 数据 | 国家资源、首都粮仓、外交解释字段、战争动员目标、持久战役梯队及 180 天满攻势准备状态 |
...
| [tests/test_suite.gd](tests/test_suite.gd) | 测试 | 783 断言，headless 运行 |
| [tests/map_visual_smoke.gd](tests/map_visual_smoke.gd) | 视觉烟测 | 构造占领省份与攻势事件，用 Godot Movie Maker 验证真实渲染路径 |
| [tests/ai_longrun.gd](tests/ai_longrun.gd) | 诊断 | 4 种子 × 1095 天 AI 长跑，统计净占领、实际易手、满准备攻势及占城后二阶段动作；支持旧版固定城防 A/B |
...
- **性能缓存**：`AiWorldView` 建立 `armies_by_city` 与驻军战力索引；AI 决策周期复用 Dijkstra 路径场；`ThreatField` 复用只依赖道路拓扑的传播距离；补给从粮仓反向构建国家网络；`EquivariantOrder` 缓存城市物理 rank。加入满准备攻势后，2026-08-03 headless 实测 1095 天每种子约 31.0～45.8 秒，四种子约 166 秒。
...
- 求和使用双边效用：至少一方达到 `1.25` 提议阈值，另一方达到 `0.60` 接受底线，且双方结算后意愿合计至少 `2.50`；实际占领收益、失地止损、战争目标兑现/失守、资源危机、战力差和战争疲劳都进入双方分数。战争满 900 天后强制允许停战，停战期 180 天。
...
- 战争中每 60 天进入下一轮国家级战役规划窗口：若目标仍有效且兵力已齐则立即发动，否则先重新集结；目标已占领时重新选择敌方前线目标。恢复窗口内的本国法理失地优先于原进攻目标，反攻不等待普通 60 天窗口。单轮计划按目标需求拆成持久梯队，首梯队只投入最小充分兵力，后续梯队在前梯队接战后连续投入。后续梯队继承整轮备战倍率与持续天数，并从各自实际投入日开始计时。
- 60 天窗口到期时，国家会用当前准备倍率重算“集结军预计攻击战力 / 目标 60 日威胁”。若仍低于 `ATTACK_ENTER_RATIO / ai_aggression`，则冻结该目标并继续准备至 180 天；普通 Utility 军队在此期间不得零散抢攻。第 180 天统一以 `2.0×` 攻击加成发动，既有兵力/补给/路径硬门槛仍保留；近期法理失地反攻不等待该周期。
...
| 正式 AI 只有固定进攻性，无法配置国家风险偏好；纯僵持时宣战与外交动作不足 | `Nation.ai_aggression` 成为国家级唯一配置源，正式模式按 `0.5～1.5` 截断后同时影响宣战收益、战术进攻/撤退阈值和攻势重整周期；默认 `1.0` 保持镜像一致。当前全局宣战线为 `1.00`，国家级攻势周期为 60 天；资源、粮食和路径硬门槛不变 |
...
| 均势战争中普通 Utility 加分始终无法跨过攻击阈值，国家长期只有驻防而无破局动作 | 国家攻势在 60 天窗口用实际集结军、目标威胁及当前准备倍率评估；暂时不足则进入显式 180 天满准备计划，期间持续集结并禁止零散攻击，到期以 2 倍攻击加成统一发动 |
...
- 快速回归 `./run_tests.sh` = **783 passed / 0 failed**（含无限正面混编火力等价、5000 正面完整多回合拆分轨迹、双边求和、按准备天数延长加成、满准备占城后二阶段、城防恢复、严格回放与镜像等变）。
...
- 真实地图 4 种子 × 1095 天：全部 4 国存活，`invalid=0`、`commit_failures=0`、`starving=0`；`net_captures=9`、`turnovers=34`、wars 10、peace 10、offensives 69，其中 **17 次**为 2 倍满准备攻势。真实二阶段选择为 0 次追击、0 次驻边、6 次驻城。相较修复前 36 次易手，消除拆分轮换额外战力后易手降至 34，但未出现战争僵死。
