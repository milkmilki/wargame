...
| [scripts/model/nation.gd](scripts/model/nation.gd) | 数据 | 国家资源、首都粮仓、外交解释字段、战争动员目标、唯一州治目标与精简州战役计划 |
...
- **州级战役滞回**：唯一战略目标写入 `campaign_objective_center_city`，执行状态只保存在 `administrative_campaign_plan`。目标州仍可达时不跨州换目标；道路、州域或控制状态变化时按版本指纹重建州内战术计划。
...
- 战争执行统一为州级状态机。每国只经营一个州治战略目标，`C < R+V` 时夺取接壤属府或驻守等待，`C >= R+V` 时沿州内最短走廊攻州治，州治控制后再清理本州残余府。每轮最多新增三支主战军，达到需求后立即停止全国抽调。
...
| 国家攻势目标和军队归属口径不一致 | `Nation.administrative_campaign_plan` 只保存一个州治目标，并以 `army_assignments` 保证一军只属于一个州战役；兵力需求统一读取 `C/R/V`，不再维护并行城市目标、波次或梯队 |
...
| 州治守军与援军需求过高时，双方可能长期僵持 | 进攻方按 `R+V-C` 增援，并通过占府与断粮降低 `R`；防守方按敌军 `C` 集结野战军，兵力不足时驻州等待，达标后主动出城决战 |
...
- **改部署迟滞** → 只动 `DEFENSIVE_DEPLOYMENT_LOCK_DAYS`。防御锁保留不同方向的真实紧急解锁，但不得让持续邻敌绕过 `defensive_blocked_edge_*` 返回刚撤离的同一边。
- **改备战或州战役计划** → 备战取消迟滞真源是 `Nation.war_preparation_unready_since_day`；宣战后唯一战略目标是 `campaign_objective_center_city`，军队分配与阶段真源是 `administrative_campaign_plan`。所有进攻、等待和增援都必须能由同一组 `C/R/V` 查询解释。
...
- 既有州战役计划每轮按 `R+V-C` 增量补充军队，不清空在途军；
- 备战就绪只检查目标州治及其合法集结入口；
