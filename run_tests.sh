...
echo "==> [1/24] 编译检查（headless 导入，捕获脚本错误）"
...
echo "==> [5/24] 贸易联通 gate-context 等价门禁"
HOME="$GODOT_HOME" "$GODOT" --headless --path "$PROJECT_DIR" \
...
echo "==> [7/24] 国内 ideal field cache 等价门禁"
HOME="$GODOT_HOME" "$GODOT" --headless --path "$PROJECT_DIR" \
