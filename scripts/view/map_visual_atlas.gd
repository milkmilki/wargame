...
static func build_visual_atlas(
	...
	var elevation := Image.create(
		safe_size.x, safe_size.y, false, Image.FORMAT_RF
	)
	var land := Image.create(safe_size.x, safe_size.y, false, Image.FORMAT_RF)
	var city_id := Image.create(
		safe_size.x, safe_size.y, false, Image.FORMAT_RF
	)
	city_id.fill(Color(-1.0, 0.0, 0.0, 1.0))
	var region_edge := Image.create(
		safe_size.x, safe_size.y, false, Image.FORMAT_RF
	)
	var coast := Image.create(safe_size.x, safe_size.y, false, Image.FORMAT_RF)
	...
	_fill_geography(height_image, elevation, land, safe_size)
...
static func sample_height_uv(atlas: Dictionary, uv: Vector2) -> float:
	return _sample_channel(atlas.get("elevation"), uv, 0.0)
...
static func _fill_geography(
	height_image: Image, elevation: Image, land: Image, size: Vector2i
) -> void:
	...
	for y in range(size.y):
		...
		for x in range(size.x):
			...
			var signed_height := TerrainMapGenerator.packed_signed_elevation(color)
			elevation.set_pixel(x, y, Color(signed_height, 0.0, 0.0, 1.0))
			if TerrainMapGenerator.packed_is_land(color):
				land.set_pixel(x, y, Color(1.0, 0.0, 0.0, 1.0))


static func _fill_city_ids(
	game_state: GameState, city_id: Image, land: Image, size: Vector2i
) -> void:
