...
## 藩王展示随当前陆城规模变化：不足五城采用藩都全称，五城起采用
## 藩都地域字；两者都追加“王”。基础 Nation.name 保留建立时的稳定值。
static func _vassal_display_name(game_state, nation_id: int) -> String:
	if not _valid_nation_id(game_state, nation_id):
		return ""
	var owned := _owned_land_city_ids(game_state, nation_id)
	var capital_id := int(game_state.nations[nation_id].capital_city_id)
	if (
	...
	if capital_id < 0:
		var stored := str(game_state.nations[nation_id].name).strip_edges()
