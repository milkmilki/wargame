...
const OFFENSIVE_BONUS_MAX_PREPARATION_DAYS: int = 0
const OFFENSIVE_BONUS_MAX_MULTIPLIER: float = 1.0

static func offensive_preparation_multiplier(_preparation_days: int) -> float:
	return 1.0

static func offensive_bonus_duration_days(_preparation_days: int) -> int:
	return 0
...
## 国家级攻势以一次正式地图 AI 决策周期为基础波次。AI 仍按性格缩放，
## 且必须完成实际集结/战力门槛；这里只缩短成功攻势后的重整空窗。
const CAMPAIGN_OFFENSIVE_INTERVAL_DAYS: int = AI_DECISION_INTERVAL_DAYS
...
const CAMPAIGN_ATTACK_ENTER_RATIO: float = 1.25
const CAMPAIGN_TARGET_COMMIT_RATIO: float = 3.00
const CAMPAIGN_STAGED_TROOP_RATIO: float = 1.00
...
## 每支15000人主战军都是独立指挥单位。单次国家级攻势仍最多调动六支，
## 其余部队留作防御和后续波次，不改写既有攻势规划逻辑。
const CAMPAIGN_MAX_COMMAND_UNITS: int = BattleGroup.MAX_CAMPAIGN_COMMAND_UNITS
...
func _campaign_objective_defense_power(
	...
	if state.recognized_owner_of(objective_city) != nation_id:
		defense_power += ArmyPower.city_garrison_defense(
			state, nation_id,
			state.administrative_center_of(objective_city)
		)
	return defense_power


func _campaign_attack_ratio_threshold(_nation_id: int) -> float:
	return CAMPAIGN_ATTACK_ENTER_RATIO
...
func _campaign_offensive_interval(nation_id: int) -> int:
	...
	return int(round(
		float(CAMPAIGN_OFFENSIVE_INTERVAL_DAYS)
			* RulerProfile.offensive_interval_multiplier(nation)
			/ aggression
	))
...
func _nearby_main_reinforcement_candidates(
	...
	for army in state.armies:
		if (
			...
			or nation.campaign_preparation_assignments.has(army.id)
			or nation.campaign_attack_assignments.has(army.id)
...
func _campaign_force_recruitment_demand(
	...
	if active_offense:
		...
		var demands := (
			campaign_allocation.target_demands
			if campaign_allocation != null else {}
		)
		if demands.is_empty():
			for target_city in force_demand_targets:
				var fallback_demand := _campaign_target_group_demand(
					view.nation_id, target_city, threat
				)
				if not fallback_demand.is_empty():
					demands[target_city] = fallback_demand
		for target_city in demands:
			var target_demand := demands[target_city] as Dictionary
			# required_power 已按满准备倍率折算；扩编容量还原为原始战力，
			# 避免单位在尚未获得进攻加成的集结阶段被守军提前耗尽。
			target_main_capacity += maxi(
				maxi(
					int(ceil(
						float(target_demand.get("required_manpower", 0))
							* CAMPAIGN_TARGET_COMMIT_RATIO
					)),
					int(ceil(
...
func _assign_grid_campaign_target(
	...
	if state.recognized_owner_of(target_city) != nation_id:
		grid_target_defense += ArmyPower.city_garrison_defense(
			state, nation_id,
			state.administrative_center_of(target_city)
		)
...
func _plan_campaign_allocation(
	...
	var assigned_power := {}
	var required_manpower_by_target := {}
	var required_power_by_target := {}
	for target_city in plan.candidate_target_ids:
		plan.target_group_budget[target_city] = 0
		var demand: Dictionary = plan.target_demands.get(
			target_city, {}
		)
		required_manpower_by_target[target_city] = int(
			demand.get("required_manpower", 1)
		) * CAMPAIGN_TARGET_COMMIT_RATIO
		required_power_by_target[target_city] = float(
			demand.get("required_power", 1.0)
		) * OFFENSIVE_BONUS_MAX_MULTIPLIER
	...
	for target_city in plan.candidate_target_ids:
		var required_manpower := float(
			required_manpower_by_target.get(target_city, 1.0)
		)
		var required_power := float(
			required_power_by_target.get(target_city, 1.0)
		)
		while used_groups.size() < CAMPAIGN_MAX_COMMAND_UNITS:
			if (
				float(assigned_manpower.get(target_city, 0.0))
					>= required_manpower
			...
			var group_id := int(best["group_id"])
			used_groups[group_id] = true
			plan.target_group_budget[target_city] = int(
	...
	plan.desired_group_count = used_groups.size()
	plan.required_group_count = used_groups.size()
...
func _evaluate_campaign_group_for_target(
	...
	for army in group_members:
		...
		for staging_city in staging:
			var edge := state.edge_of(staging_city, target_city)
			if edge != null and edge.max_manpower > 0:
...
func _launch_group_campaign_offensive(
	...
	var nation := state.nations[nation_id]
	_reconcile_campaign_active_wave(nation_id)
	# 当前模型只有一个 active-wave 容器；上一波仍执行时不能覆盖其占领续接。
...
func _make_campaign_attack_order(
	...
	var attack := ActionCandidate.make(
		ActionCandidate.Kind.ATTACK,
		2000.0,
		"国家战役第%d波：军%d按统一计划准备%d天攻击城市%d"
			% [wave_number, army_id, preparation_days, target_city],
		target_city
	)
...
## 既有战役计划的执行续接。前梯队进入目标城市后即开放下一梯队；若前梯队
## 提前失去作战能力，仍沿用次日接替规则。
...
func _manage_campaign_offensive(
	...
	if (
		...
		if next.is_empty():
			return false
		objective_center = int(next.get(
			"administrative_center_city_id", next.get("city_id", -1)
		))
		objective_city = _administrative_tactical_objective(
			nation_id, defender_id, objective_center, next
		)
		nation.campaign_objective_center_city = objective_center
	...
	for target_city in preparation_targets:
		...
		if not can_launch and not legal_reclamation:
			continue
		var required := _campaign_minimum_staged_troops(
			nation_id, target_city
		)
...
func _manage_administrative_campaign(
	...
	if plan == null or plan.center_city_id != center_city_id:
...
func _administrative_tactical_objective(
	nation_id: int,
	defender_id: int,
	center_city_id: int,
	objective: Dictionary = {}
) -> int:
	var cached := int(objective.get("tactical_city_id", -1))
	if (
		cached >= 0
		and cached < state.cities.size()
		and state.administrative_center_of(cached) == center_city_id
		and state.cities[cached].owner_nation == defender_id
	):
		return cached
	return DiplomacyAI.administrative_tactical_target(
		state, nation_id, defender_id, center_city_id
	)
...
## SIEGE 状态机（每天一 tick）。城市持久守军以临时 Army 参加普通战斗；
## 守军与普通防守军清空后立即破城。第三方挑战和联盟解围仍沿用围城外壳。
func _advance_siege(
	battle: Battle,
	shared_roll: int = -1,
	...
	# 阶段 1：城市防卫共同体。持久守军只在本回合临时进入 side_b。
	if (
		battle.side_b_defends_city
		or (
			battle.side_b.is_empty()
			and battle.city != null
		...
		if not battle.finished:
			return
		var city_garrison_destroyed := (
			battle.city == null
			or battle.city.garrison_manpower <= 0
		)
		if battle.winner_side != 1 and not city_garrison_destroyed:
...
func _attach_city_garrison(battle: Battle) -> Army:
	...
	garrison.city_garrison_combat_multiplier = (
		state.city_garrison_efficiency(attacker_id, battle.city.id)
...
func _complete_siege_capture(battle: Battle) -> void:
	...
	_capture_city(captor, battle.city, claimant, false)
	for army in battle.side_a:
		if army.size <= 0:
			army.battle_id = -1
			continue
	...
	if captor != null and captor.size > 0 and state.has_military_access(
		captor.owner_nation, battle.city.owner_nation
	):
		_execute_campaign_post_capture_plan(captor, battle.city)
	battle.finished = true
...
func _capture_city(
	...
	if execute_post_capture_plan and army != null and captor_can_remain:
...
func _execute_campaign_post_capture_plan(
	...
	if route_valid and execution_army != null:
		route_valid = route_edge.max_manpower > 0
	if (
		route_valid
		...
		if _execute_ai_candidate(execution_army, attack):
			_extend_campaign_post_capture_plan(
				execution_army,
				next_city,
				plan
			)
...
func _extend_campaign_post_capture_plan(
	army: Army,
	next_city: int,
	...
	if (
		army == null
		or next_city < 0
		or next_city >= state.cities.size()
	):
		return
	var enemy_nation := int(previous_plan.get("enemy_nation", -1))
	var next_step := _campaign_post_capture_target(
		army,
		state.cities[next_city],
		null,
		enemy_nation
	)
	if (
		next_step.is_empty()
		or float(next_step.get("attack_ratio", 0.0))
			< _campaign_attack_ratio_threshold(army.owner_nation)
	):
		return
	state.nations[army.owner_nation].campaign_post_capture_plans[
		next_city
	] = {
		"next_city": int(next_step["city_id"]),
		"group_id": int(previous_plan.get("group_id", -1)),
		"heavy_army_id": int(
			previous_plan.get("heavy_army_id", -1)
		),
		"execution_army_id": army.id,
		"enemy_nation": enemy_nation,
		"created_day": state.day,
		"steps": int(previous_plan.get("steps", 1)) + 1,
	}
...
func _campaign_post_capture_target(
	...
	for target_id in neighbors:
		...
		var mobile_defense := 0.0
		if threat != null:
			mobile_defense = threat.threat_at(target_id)
		else:
			for defender in state.armies_at_city(target_id):
				if state.is_enemy(
					army.owner_nation,
					defender.owner_nation
				):
					mobile_defense += ArmyPower.effective(defender)
		var defense_power := (
			mobile_defense + ArmyPower.city_garrison_defense(
				state, army.owner_nation,
				state.administrative_center_of(target.id)
			)
		)
		var attack_ratio := (
			ArmyPower.effective(army)
