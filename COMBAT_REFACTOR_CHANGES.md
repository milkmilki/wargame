...
> 含 `0.50～0.60` 黄河、真实边距与双河道硬阻隔的正式地图 4 种子 × 1095 天：`net_captures=116`、`turnovers=158`、`wars=12`、
> `offensives=71`、`multi_prep=11`、`max_parallel=3`，
...
- 合计 `net_captures=116`、`turnovers=158`、`wars=12`、`offensives=71`，既有多城攻势也有邻接迟滞和主会战，没有形成静态僵局。
- 出现 11 个真实多目标准备计划，单国同轮最大实际并行方向数为 3；各种子 `eliminated_wars=0`，且 `invalid=0`、`commit_failures=0`，稳定超过 30 天的军制偏差为 0；总耗时约 `95.2s`。
