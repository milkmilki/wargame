...
- 均势破局：州战役按 `R+V-C` 持续增援；不足时占府、封锁或等待恢复，达标后攻州治，不再通过准备时长制造隐藏攻击倍率。
- 攻势延续：州治控制后进入本州清理阶段，最多同时清理两个敌控府，不跨州冻结续攻路线。
...
| `Nation.campaign_objective_center_city/administrative_campaign_plan`（新增） | 保存唯一州治战略目标与州内阶段、战术目标、军队分配、失败冷却和版本指纹。旧多目标准备、波次、梯队及续攻字段已删除。 |
